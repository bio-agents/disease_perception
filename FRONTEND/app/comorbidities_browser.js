...
// Agenttips attached to graph elements
...
export class ComorbiditiesBrowser {
	constructor(setup) {
		...
		// The body of the search agenttip, to be used by tippy
	...
	initializeControls() {
		...
		// Static agenttip view
		$controls.append('<span class="label label-default">Selected element</span>');
		this.ui.$agenttipView = $('<div class="agenttip-view"><i>(None)</i></div>');
		$controls.append(this.ui.$agenttipView);
	...
	populateNodeAgenttip(node) {
		if(!node.scratch('agenttip')) {
			let content = this.currentView.makeNodeAgenttipContent(node);
			...
			node.scratch('agenttip',$(content).clone());
		}
		
		return node;
	}
	
	cachedNodeAgenttip(node) {
		return this.populateNodeAgenttip(node).scratch('agenttip');
	}
	
	cachedNodeTippy(node) {
		return this.populateNodeAgenttip(node).scratch('tippy');
	}
	
	populateEdgeAgenttip(edge) {
		if(!edge.scratch('agenttip')) {
			let content = this.currentView.makeEdgeAgenttipContent(edge);
			...
			edge.scratch('agenttip',$(content).clone());
		}
		
		return edge;
	}
	
	cachedEdgeAgenttip(edge) {
		return this.populateEdgeAgenttip(edge).scratch('agenttip');
	}
	
	cachedEdgeTippy(edge) {
		return this.populateEdgeAgenttip(edge).scratch('tippy');
	}
		...
		let $ctxNodeHandler = this.ctxHandlers.nodes.length > 0 ?
			(evt) => {
				...
				if(evt.originalEvent.ctrlKey) {
					...
					this.ui.$agenttipView.html(this.cachedNodeAgenttip(evt.target));
				}
			}
			:
			(evt) => {
			...
			let $ctxEdgeHandler = this.ctxHandlers.edges.length > 0 ?
				(evt) => {
					...
					if(evt.originalEvent.ctrlKey) {
						...
						this.ui.$agenttipView.html(evt.target.scratch('agenttip'));
					}
				}
				:
				(evt) => {
